...
$filename = $oc_paths['site_uploads'] . $type->getFileHash($_GET['categoryID'], $_GET['UID'], $_GET['indicatorID'], $value);
...
if (in_array($fileExtension, $imageExtensionWhitelist) && file_exists($filename))
{
    ...
    else
    {
        ...
        {
            if (file_exists($oc_paths['site_uploads'] . 'img_' . $origFile)
                && filemtime($oc_paths['site_uploads'] . 'img_' . $origFile) > time() - 604800)
            {
                readfile($oc_paths['site_uploads'] . 'img_' . $origFile);
            }
            else
            {
                ...
                if ($width > 500)
                {
                    ...
                    if ($src !== false)
                    {
                        ...
                        imagejpeg($newImg, $oc_paths['site_uploads'] . 'img_' . $origFile, 90);
                        readfile($oc_paths['site_uploads'] . 'img_' . $origFile);
                    }
                    else
                    {
