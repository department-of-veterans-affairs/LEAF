      ...
      <div class="printmainblock<!--{if ($indicator.required == 0 && $indicator.data == '') || $indicator.format == 'json'}--> notrequired<!--{/if}-->">
        <div class="printmainlabel">
            ...
            <!--{if $date < $indicator.timestamp && $date > 0}-->
                <img src="<!--{$libsPath}-->dynicons/?img=appointment.svg&amp;w=16" alt="View History" title="View History" style="cursor: pointer" onclick="getIndicatorLog(<!--{$indicator.indicatorID|escape}-->); $('#histdialog1').dialog('open')" />&nbsp;
            <!--{/if}-->
            <!--{if $indicator.isWritable == 0}-->
                <img src="<!--{$libsPath}-->dynicons/?img=emblem-readonly.svg&amp;w=16" alt="Read-only" title="Read-only" />&nbsp;
            <!--{else}-->
                <img src="<!--{$libsPath}-->dynicons/?img=accessories-text-editor.svg&amp;w=16" alt="Edit this field" title="Edit this field" style="cursor: pointer" />&nbsp;
            <!--{/if}-->
            </div>
            <!--{if $indicator.isWritable == 0}-->
