...
nationalEmployeeSelector.prototype.initialize = function() {
	...
	var labelText = $("[for='" + id + "']").text().trim();
	...
	$('#' + this.containerID).html('<div id="'+this.prefixID+'border" class="employeeSelectorBorder">\
			...
			<input id="'+this.prefixID+'input" type="search" class="employeeSelectorInput" style="width: calc(90% - 100px)" aria-label="Search for user to add as ' + arialLabelText + '"></input></div>\
	...
	var ajaxOptions = {
		...
		success: function(response) {
			...
			if(t.outputStyle == 'micro') {
				buffer = '<table aria-live="true" aria-atomic="true" class="employeeSelectorTable"><thead><tr><th>Name</th><th>Contact</th></tr></thead><tbody id="' + t.prefixID + 'result_table"></tbody></table>';
			}
			else {
				buffer = '<table aria-live="true" aria-atomic="true" class="employeeSelectorTable"><thead><tr><th>Name</th><th>Location</th><th>Contact</th></tr></thead><tbody id="' + t.prefixID + 'result_table"></tbody></table>';
			}
