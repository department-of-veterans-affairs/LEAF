...
foreach($portal_records as $rec) {
    ...
    try {
        ...
        foreach ($tables_to_update as $table_name) {
            ...
            $batchcount = 0;
            //rows in offsets. 10000 rows if data_history (high dups), 1000 for others
            while(count($resBatch = getUniqueIDBatch($pdb, $table_name, $field_name, $batchcount)) > 0) {
                $batchcount += 1;

                //remove duplicates and entries that already have metadata added
                $mapAdded = array();
                foreach ($resBatch as $rec) {
                    $userID = strtoupper($rec['userID']);
                    if(!isset($mapAdded[$userID]) && !isset($rec[$field_name])) {
                        $mapAdded[$userID] = 1;
                    }
                }

                $resUniqueIDsBatch = array_keys($mapAdded);
