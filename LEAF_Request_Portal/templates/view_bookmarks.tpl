...
<!--
    ...
    <!--{if $is_service_chief == true}-->
        <div><a href="?a=service_chief" style="text-decoration: none"><img src="<!--{$libsPath}-->dynicons/?img=accessories-text-editor.svg&amp;w=32" style="float: left; padding: 2px" alt="Service Prioritization" title="Service Prioritization" /> Service Prioritization for Equipment/FTE</a></div>
    <!--{/if}-->
    <!--{if $ingroup_quadrad == true}-->
    <div><a href="?a=quadrad_equipment_review" style="text-decoration: none"><img src="<!--{$libsPath}-->dynicons/?img=utilities-system-monitor.svg&amp;w=32" style="float: left; padding: 2px" alt="Quadrad Prioritization" title="Quadrad Prioritization" /> Quadrad Equipment Prioritization</a></div>
    <div><a href="?a=quadrad_fte_review" style="text-decoration: none"><img src="<!--{$libsPath}-->dynicons/?img=system-users.svg&amp;w=32" style="float: left; padding: 2px" alt="Quadrad Prioritization" title="Quadrad Prioritization" /> Quadrad FTE Prioritization</a></div>
    <!--{/if}-->
    </div>
-->
</div>
...
<div id="bookmark_<!--{$record.recordID|strip_tags}-->" style="float: left; border: 1px solid black; margin: 8px; padding: 0px; background-color: <!--{$record.stepBgColor|strip_tags}-->; color: <!--{$record.stepFontColor|strip_tags}-->; width: 370px">
    <div style="float: left; cursor: pointer; background-color: black; font-size: 150%; font-weight: bold; color: white; padding: 8px; text-align: center" onclick="window.location='?a=printview&amp;recordID=<!--{$record.recordID|strip_tags}-->'">
        ...
        <img src="<!--{$libsPath}-->dynicons/?img=<!--{$record.actionIcon|strip_tags}-->&amp;w=32" alt="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" title="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" />
        <!--{else}-->
        <img src="<!--{$libsPath}-->dynicons/?img=emblem-notice.svg&amp;w=32" alt="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" title="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" />
        <!--{/if}-->
    </div>
    <div>
        <div style="background-color: #e0e0e0; font-weight: bold; border-bottom: 1px solid black; padding: 2px">
                        <span style="float: right; cursor: pointer"><img src="<!--{$libsPath}-->dynicons/?img=process-stop.svg&amp;w=16" alt="Delete Bookmark" title="Delete Bookmark" onclick="removeBookmark(<!--{$record.recordID|strip_tags}-->)"/></span>
...
<img src="<!--{$libsPath}-->dynicons/?img=help-browser.svg&amp;w=96" alt="empty" style="float: left"/><span style="font-size: 200%"> You do not have any requests bookmarked!<br /><br />To bookmark a request, open a request and select "Add Bookmark".</span>
</div>
<!--{/if}-->
