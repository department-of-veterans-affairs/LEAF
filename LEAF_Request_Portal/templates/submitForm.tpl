...
<div style="padding: 8px; width: 260px; margin: auto" id="submitControl">
    <button class="buttonNorm" type="button" style="font-weight: bold; font-size: 120%" onclick="doSubmit({$recordID});"><img src="../libs/dynicons/?img=go-next.svg&amp;w=32" alt="Submit" /> {if $lastActionTime > 0}Re-{/if}Submit {$requestLabel}</button>
</div>