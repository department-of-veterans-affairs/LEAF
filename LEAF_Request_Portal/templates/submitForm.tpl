...
<div style="padding: 8px; width: 260px; margin: auto" id="submitControl">
    <button class="buttonNorm" type="button" style="font-weight: bold; font-size: 120%" title="Submit Form" onclick="doSubmit({$recordID|strip_tags});"><img src="dynicons/?img=go-next.svg&amp;w=32" alt="Submit" /> {if $lastActionTime > 0}Re-{/if}Submit {$requestLabel|sanitize}</button>
</div>