...
<button type="button" id="fileAdditional" class="buttonNorm" onclick="$('#fileIframe_{$recordID|strip_tags}_{$indicatorID|strip_tags}_{$series|strip_tags}').css('display', 'block'); $('#fileAdditional').css('visibility', 'hidden')"><img src="{$lib_path}dynicons/?img=document-open.svg&amp;w=32" /> Attach Additional File</button>
