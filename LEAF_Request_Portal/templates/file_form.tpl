<form id="record" enctype="multipart/form-data" action="ajaxIndex.php?a=doupload&amp;recordID={$recordID|strip_tags}" method="post">
    ...
    <input id="file{$indicatorID|strip_tags}" name="{$indicatorID|strip_tags}" type="file" aria-labelledby="format_label_{$indicatorID|strip_tags}" /></div>
    <div id="file{$indicatorID|strip_tags}_status" style="visibility: hidden; display: none; background-color: #fffcae; padding: 4px"><img src="{$absPortPath}/images/indicator.gif" alt="loading..." /> Attaching file...</div>
