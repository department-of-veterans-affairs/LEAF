<div><a href="?a=status&amp;recordID=<!--{$recordID|strip_tags}-->"><img src="<!--{$lib_path}-->dynicons/?img=printer.svg&amp;w=16" alt="Print status" /></a></div>
<div> <!-- main content -->
...
<!--{foreach from=$dependencies item=dependency}--><!--{strip}-->
    <!--{if $dependency.filled==1}-->
        <span style="padding: 4px; margin: 4px; color: green; font-weight: bold"><img class="print" src="<!--{$lib_path}-->dynicons/?img=dialog-apply.svg&w=16" alt="checked" /> <!--{$dependency.description|sanitize}--></span><br />
    <!--{elseif $dependency.filled==0}-->
        <span style="padding: 4px; margin: 4px; color: gray">[ ? ] <!--{$dependency.description|sanitize}--> (Pending)</span><br />
    <!--{else}-->
        <span style="padding: 4px; margin: 4px; color: red"><img class="print" src="<!--{$lib_path}-->dynicons/?img=process-stop.svg&w=16" alt="not checked" /> <!--{$dependency.description|sanitize}--></span><br />
    <!--{/if}-->
<!--{/strip}--><!--{/foreach}-->
</div>
