...
if (isset($_COOKIE['REMOTE_USER']))
{
    ...
    if (count($res) > 0) {
        ...
        $sql = 'INSERT INTO `employee` (`firstName`, `lastName`, `middleName`,
                ...
                VALUES (:firstName, :lastName, :middleName, :userName,
                        :phoFirstName, :phoLastName, :domain, :lastUpdated,
                        :new_empUUID)
    ...
    header('Location: ' . $protocol . AUTH_CERT_URL . '/?r=' . base64_encode($_SERVER['REQUEST_URI']));
    exit();
}

// No REMOTE_USER cookie - redirect to auth
header('Location: ' . $protocol . AUTH_CERT_URL . '/?r=' . base64_encode($_SERVER['REQUEST_URI']));
exit();
