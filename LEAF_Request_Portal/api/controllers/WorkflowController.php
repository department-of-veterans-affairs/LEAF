...
class WorkflowController extends RESTfulResponse
{
    ...
    public function get($act)
    {
        ...
        $this->index['GET']->register('workflow/step/[digit]/actions', function ($args) use ($workflow) {
            return $workflow->getStepActions((int)$args[0]);
        });
        ...
        $this->index['GET']->register('workflow/[digit]/workflowStepDependencies', function ($args) use ($workflow) {
            $workflow->setWorkflowID($args[0]);
            return $workflow->getWorkflowStepDependencies();
        });
