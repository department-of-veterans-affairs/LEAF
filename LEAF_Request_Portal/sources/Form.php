...
class Form
{
    ...
    public function getIndicator(int $indicatorID, int $series, int $recordID = null, bool $parseTemplate = true): array
    {
        ...
        if (
            $data[0]['format'] == 'orgchart_employee'
            && !empty($data[0]['data'])
        ) {
        ...
        if (
            $data[0]['format'] == 'orgchart_position'
            && isset($data[0]['data'])
        ) {
        ...
        if ($parseTemplate) {
            $form[$idx]['html'] = str_replace(
                ['{{ iID }}', '{{ recordID }}', '{{ data }}'],
                [$idx, $recordID, $form[$idx]['value']],
                $data[0]['html']
            );
            $form[$idx]['htmlPrint'] = str_replace(
                ['{{ iID }}', '{{ recordID }}', '{{ data }}'],
                [$idx, $recordID, $form[$idx]['value']],
                $data[0]['htmlPrint']
            );
    ...
    public function deleteRecord(int $recordID)
    {
        ...
        $submittedRecordsVars = array(':recordID' => $recordID);
        $submittedRecordsSQL = 'SELECT recordID, submitted, stepID FROM records
        LEFT JOIN records_workflow_state USING (recordID)
        WHERE recordID=:recordID
            AND submitted > 0';
        $submittedRecordsRes = $this->db->prepared_query($submittedRecordsSQL, $submittedRecordsVars);
        if (
            isset($submittedRecordsRes[0])
            && $submittedRecordsRes[0]['stepID'] == null
        ...
        $deleteSQL = 'UPDATE records SET
        deleted=:time
        WHERE recordID=:recordID';
        $this->db->prepared_query($deleteSQL, $deleteVars);

        // actionID 4 = delete
        $actionHistoryInsertVars = array(
            ':recordID' => $recordID,
        ...
        $actionHistoryInsertSQL = 'INSERT INTO action_history (recordID, userID, dependencyID, actionType, actionTypeID, time)
        VALUES (:recordID, :userID, :dependencyID, :actionType, :actionTypeID, :time)';
        $res = $this->db->prepared_query($actionHistoryInsertSQL, $actionHistoryInsertVars);

        // delete state
        $deleteWorkflowStateVars = array(':recordID' => $recordID);
        $deleteWorkflowStateSQL = 'DELETE FROM records_workflow_state WHERE recordID=:recordID';
        $this->db->prepared_query($deleteWorkflowStateSQL, $deleteWorkflowStateVars);

        // delete tags
        $deleteTagsVars = array(':recordID' => $recordID);
        $deleteTagsSQL = 'DELETE FROM tags WHERE recordID=:recordID';
        $this->db->prepared_query($deleteTagsSQL, $deleteTagsVars);
    ...
    public function checkReadAccess(array $records): array
    {
        if (count($records) > 0) {
            ...
            if (count($res) == 0 || $this->login->checkGroup(1)) {
                ...
                if ($countPurged > 0 && headers_sent() == false) {
    ...
    private function isProcessQuery(array $query): bool
    {
        ...
        if (isset($query['limit']) && is_numeric($query['limit'])) {
            ...
            } else if ($query['limit'] <= 10000 && (!empty($query['getData']) && count($query['getData']) >= 10)) {
                $externalProcessQuery = true;
            }
        } else {
    ...
    private function storeQueryForProcess(string $inQuery): array
    {
        ...
        $user = $dir->lookupLogin($_SESSION['userID'], true);

        $vars = [
            ':lastProcess' => time()
        ];

        $processQueryTotalSQL = 'SELECT id FROM process_query WHERE lastProcess <= :lastProcess';
        $processQueryTotalRes = $this->db->prepared_query($processQueryTotalSQL, $vars);
        $totalToProcess = (count($processQueryTotalRes) > 0) ? count($processQueryTotalRes) : 1;
        ...
        $processQueryCheckSQL = 'SELECT id, userID, `url`, lastProcess FROM process_query WHERE userID = :userID and  `url` = :url';
        $processQueryCheckRes = $this->db->prepared_query($processQueryCheckSQL, $vars);

        if (count($processQueryCheckRes) == 0) {
            // add in for the insert if we are to insert.
            $vars[':lastProcess'] = 0;

            $insertProcessQuerySQL = 'INSERT INTO process_query ( userID, `url`, lastProcess) 
        VALUES (:userID, :url, :lastProcess)';

            $this->db->prepared_query($insertProcessQuerySQL, $vars);
    ...
    private function outputFile(string $inQuery): void
    {
        ...
        if (!empty($processQueryCheckRes) && $processQueryCheckRes[0]['lastProcess'] > 0) {
            ...
            if ($currentProcess['lastProcess'] > 0 && is_file($currentFileName)) {
                ...
                header("Content-Length: " . filesize($currentFileName));
    ...
    private function processQueryData(array $res, array $compactedVariables): array
    {
        ...
        if (!empty($recordIDs)) {
            ...
            if ($joinRecords_Step_Fulfillment) {
                $strSQL = 'SELECT stepID, recordID, fulfillmentTime, workflowID, stepTitle, stepBgColor, stepFontColor, stepBorder, jsSrc, posX, posY, indicatorID_for_assigned_empUID, indicatorID_for_assigned_groupID, requiresDigitalSignature,stepData FROM records_step_fulfillment LEFT JOIN workflow_steps USING (stepID) ' .
                    'WHERE recordID IN (' . $recordIDs . ')';
        ...
        if (isset($query['getData'])) {
            ...
            $retData = $this->getCustomData($data, $indicatorIDs);
        } else {
            $retData = $data;
        }
    ...
    /**
     * @param string $inQuery - the query used in the processingF
    ...
    public function query(string $inQuery, bool $processQuery = false)
    {
            ...
            switch ($q['id']) {
                ...
                case 'categoryID':
                    if ($q['operator'] != '!=') {
                        ...
                        $joins .= "LEFT JOIN (SELECT recordID, categoryID, `count` FROM category_count WHERE count > 0) lj_categoryID{$count} USING (recordID) ";
                ...
                case 'data':
                    ...
                    if ($q['indicatorID'] > 0) {
                        ...
                        $indicatorSQL = 'SELECT format, `default` FROM indicators WHERE indicatorID=:indicatorID';
                        $tResTypeHint = $this->db->prepared_query($indicatorSQL, $tVarTypeHint);
            ...
            foreach ($query['joins'] as $table) {
                switch ($table) {
                    case 'service':
                        ...
                        break;
                    case 'status':
                        $joins .= 'LEFT JOIN (SELECT recordID, stepID, blockingStepID, lastNotified, initialNotificationSent FROM records_workflow_state) lj_status USING (recordID)
        ...
        // are we processing the query this time instead of displaying to browsers?
        if ($processQuery === true) {
            ...
            // if we are storing the query for each requested process. This will allow us to pull data for each user. I  think the user tie in will be something we need to have more consideration
            ...
            if (!empty($processQueryCheckRes)) {
                ...
                // confirm the existance of the file  then update the timestamp. if it errors out I do not think the file is created until data is sent.
                if (is_file($currentFileName) === true) {
                    // update the timestamp
                    $vars = [
                        ':lastProcess' => time(),
                        ':url' => $inQuery
                    ];
                    $processQueryUpdateSQL = 'UPDATE process_query SET lastProcess=:lastProcess WHERE `url`=:url';
                    $this->db->prepared_query($processQueryUpdateSQL, $vars);
                }
            }

            return ['status' => 2, 'errors' => ["Data was processed!"]];
        } else {

            // this limit is specific to this version, the above will have a different way to handle it, maybe in the future these two can be one and the same.
            $limit = '';
            if (isset($query['limit']) && is_numeric($query['limit'])) {
                $offset = '';
                if (isset($query['limitOffset']) && is_numeric($query['limitOffset'])) {
    ...
    private function buildFormTree(int $id, int $series = null, int $recordID = null, bool $parseTemplate = true): array
    {
        ...
        if (count($res) > 0) {
            ...
            foreach ($res as $field) {
                ...
                if ($parseTemplate) {
                    $child[$idx]['html'] = str_replace(
                        ['{{ iID }}', '{{ recordID }}', '{{ data }}'],
                        [$idx, $recordID, $child[$idx]['value']],
                        $field['html']
                    );
                    $child[$idx]['htmlPrint'] = str_replace(
                        ['{{ iID }}', '{{ recordID }}', '{{ data }}'],
                        [$idx, $recordID, $child[$idx]['value']],
                        $field['htmlPrint']
                    );
