...
switch ($action) {
    ...
    case 'getindicator':
        ...
        if (is_numeric($_GET['indicatorID']))
        {
            ...
            if ($indicator[$_GET['indicatorID']]['isWritable'] == 1)
            {
                ...
                $t_form->assign('subindicatorsTemplate', customTemplate('subindicators.tpl', $config));
                $t_form->display(customTemplate('ajaxForm.tpl', $config));
            }
            else
            {
    ...
    case 'getsubmitcontrol':
        ...
        {
            $t_form->display(customTemplate('submitForm_parallel_processing.tpl', $config));
        }
        else
        {
            $t_form->display(customTemplate('submitForm.tpl', $config));
        }
    ...
    case 'printview':
        if ($login->isLogin())
        {
            ...
            if (!isset($_GET['enclosed']))
            {
                ...
                $t_form->display(customTemplate('print_form_ajax.tpl', $config));
            }
            else
            {
                $t_login = new Smarty;
                ...
                $main->assign('body', $t_form->fetch(customTemplate('print_form_ajax.tpl', $config)));
