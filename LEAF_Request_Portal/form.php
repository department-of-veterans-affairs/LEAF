...
class Form
{
    ...
    public function invalidatePastSignatures($recordID)
    {
        ...
        $res = $this->db->query_kv('SELECT * 
                                    ...
                                    AND signature_id IS NOT NULL', 'actionID', 'signature_id', $var);

        //if any signatures are found, add a `signature invalidated` to action_history and remove signatures
        if(count($res) > 0)
        {
            $actionIDArray = array_keys($res);
            $actionIDString = implode(',', $actionIDArray);
            $signatureIDArray = array_values($res);

            $this->db->beginTransaction();

            //add a `signature invalidated` to action_history
            $comment = 'actionID(s) of invalidated signatures: '.$actionIDString;
