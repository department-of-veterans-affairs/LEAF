 ...
 * @return string
 */
function smarty_modifier_sanitizeLinks($in)
{
    ...
    $in = strip_tags($in, '<a><b><i><u><ol><li><br><p><table><td><tr><a>');
    $inEscaped = htmlspecialchars($in, ENT_QUOTES);
    $in = preg_replace($pattern, $replace, $inEscaped);
    
    // scrub <a> links
    $pattern = '/&lt;a\shref\=&quot;(\S*)&quot;(\s.+)?&gt;/Ui';
    $inEscaped = preg_replace_callback($pattern,
        function($matches) {
            return '<a href="' . htmlspecialchars($matches[1], ENT_QUOTES) . '" target="_blank">';
        },
        $inEscaped);
