...
final class FormControllerTest extends DatabaseTest
{
    ...
    /**
     * Tests the `form/signatures/[digit]` endpoint.
     */
    public function testGetFormSignatures() : void
    {
        $results = self::$reqClient->get('?a=form/signatures/1');

        $this->assertNotNull($results);
        $this->assertEquals('test@email.com', $results[0][0]['data']);
        $this->assertEquals('tester', $results[0][0]['firstName']);
        $this->assertEquals('tester', $results[0][0]['lastName']);
        $this->assertEquals('1520268931', $results[0]['time']);
        $this->assertEquals('1', $results[0]['signature_id']);
    }
