...
func digsign(request *RequestData) Response {
...
func getSmartCardData(store syscall.Handle) (*SmartCardCert, error) {
	...
	procCertFreeCertificateContext.Call(raw)
...
func signFile(cert *x509.Certificate, certContext uintptr) ([]byte, []byte, string) {
	...
	ret, _, err := procGetOpenFileName.Call(uintptr(unsafe.Pointer(&ofn)))
	if ret == 0 {
...
func computeSHA256Hash(certContext uintptr, rawData []byte) (uintptr, []byte, error) {
	...
	if ret == 0 {
		procCryptDestroyHash.Call(hashHandle) // Clean up before returning error!
	...
	if ret == 0 {
		procCryptDestroyHash.Call(hashHandle)
...
func signHashWithSmartCard(certContext uintptr, hashHandle uintptr) ([]byte, error) {
	...
	if ret == 0 || keyHandle == 0 {
		return nil, fmt.Errorf(" Failed to acquire Smart Card private key. %v", err.Error())
	}
	defer func() {
		if mustFreeProvider {
...
func verifySignature(cert *x509.Certificate, certContext uintptr, computedHash, signature []byte) bool {
	...
	defer procCryptDestroyKey.Call(hPublicKey)
...
func main() {
	...
	logFile, err := os.OpenFile("digsign_host.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
	...
	for {
		...
		if err := binary.Read(os.Stdin, binary.LittleEndian, &messageLength); err != nil {
			if err == io.EOF {
				// Chrome has closed the connection
				log.Println("Connection closed by Chrome")
				break
			}
		...
		// Parse the message as a RequestData
		var request RequestData
		if err := json.Unmarshal(messageBytes, &request); err != nil {
			sendErrorResponse(fmt.Sprintf("Error parsing request: %v", err))
			continue
		}

		// Process the request
		response := digsign(&request)
...
func sendErrorResponse(errorMessage string) {
	...
	responseBytes, err := json.Marshal(response)
	if err != nil {
		log.Fatalf("Error marshaling error response: %v", err)
	}

	// Write message length
	binary.Write(os.Stdout, binary.LittleEndian, uint32(len(responseBytes)))

	// Write message
	if _, err := os.Stdout.Write(responseBytes); err != nil {
		log.Fatalf("Error writing error response: %v", err)
	}
}
...
// 		json.NewEncoder(write).Encode(helpFile())
// 		return
...
// 		json.NewEncoder(write).Encode(digsign(&req))
// 		return
// 	} else {
...
// Define a function to validate the RequestData structure
func validateRequestData(data []byte) error {
	// Define what fields are allowed in the request
	...
	for field := range requestMap {
		if !allowedFields[field] {
			return fmt.Errorf("unexpected field in request: %s", field)
		}
	}
